...
public class OAuth2RequestProp extends OAuth2PropertyModel {
    ...
    /**
     * This tells the authorization server that the application is initiating which OAuth grant
     * flow.
     *
     * @see GrantType
     */
    public static final OAuth2RequestProp RESPONSE_TYPE =
            new OAuth2RequestProp("response_type").declare(GrantType.AUTH_CODE, FlowStep.AUTHORIZE)
                                                  ...
                                                  .declare(GrantType.DEVICE_CODE,
                                                           FlowStep.EXCHANGE_TOKEN,
                                                           Constraint.OPTIONAL);

    /**
     * Tells the authorization server where to send the user back to after they approve the
     * request.
     */
    public static final OAuth2RequestProp REDIRECT_URI =
            new OAuth2RequestProp("redirect_uri").declare(GrantType.AUTH_CODE, FlowStep.AUTHORIZE)
                                                 .declare(GrantType.AUTH_CODE,
                                                          FlowStep.EXCHANGE_TOKEN)
                                                 .declare(GrantType.IMPLICIT, FlowStep.AUTHORIZE)
                                                 .declare(GrantType.PASSWORD,
                                                          FlowStep.EXCHANGE_TOKEN)
     ...
     * application. This ensures that the request to get the access token is made only from the
     * application, and not from a potential attacker that may have intercepted the authorization
     * code.
     */
    ...
    /**
     * This parameter is the authorization code that the client previously received from the
     * authorization server. It will be used to exchange for {@code access token}.
     */
