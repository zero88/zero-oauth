...
class OAuth2Properties<P extends IOAuth2PropertyMatcher> extends PropertyStore<P> implements IPropertiesFilter {
    ...
    protected OAuth2Properties(GrantType grantType, Class<P> clazz) {
        ...
        this.init(ReflectionUtils.getConstants(clazz).stream().filter(prop -> Objects.nonNull(prop.match(this.grantType))).collect(Collectors
                                                                                                                                             .toList()));
    }

    @Override
    public final List<IPropertyModel> by(FlowStep step) {
        return this.properties().stream().map(prop -> prop.match(this.grantType, step)).filter(Objects::nonNull).collect(Collectors.toList());
    }
