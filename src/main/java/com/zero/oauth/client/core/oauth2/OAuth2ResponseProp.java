...
public class OAuth2ResponseProp extends OAuth2PropertyModel implements IResponsePropModel {
    ...
    /**
     * A value that OAuth server uniquely assigns to identify the device that runs the app
     * requesting authorization. The user will be authorizing that device from another device with
     * richer input capabilities. This code lets the device running the app securely determine
    ...
    /**
     * A case-sensitive value that identifies to OAuth scopes that the application is requesting
     * access to. Your user interface will instruct the user to enter this value on a separate
     * device with richer input capabilities.
     */
    ...
    public static final OAuth2ResponseProp VERIFICATION_URL =
            new OAuth2ResponseProp("verification_url")
                    .declare(GrantType.DEVICE_CODE, FlowStep.EXCHANGE_TOKEN);
