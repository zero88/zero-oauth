...
public class ParameterConverterTest {
    ...
    public void test_ResponseProp_ParameterConverter() {
        String queryPath = "access_token=" + ACCESS_TOKEN + "&" + "token_type=" + TOKEN_TYPE + "&" +
                           "expires_in=" + EXPIRES_IN + "&" + "scope=" + SCOPE_ENCODE + "&" +
                           "state=" + STATE;
        PropertyStore<IPropertyModel> properties = new RequestParamConverter<>(responseProperties)
                .deserialize(queryPath, FlowStep.AUTHORIZE);
        assertEquals(ACCESS_TOKEN,
                     properties.get(OAuth2ResponseProp.ACCESS_TOKEN.getName()).getValue());
        assertEquals(TOKEN_TYPE,
                     properties.get(OAuth2ResponseProp.TOKEN_TYPE.getName()).getValue());
    ...
    public void test_ResponseProp_ParameterConvert_WrongSyntax() {
        ...
        new RequestParamConverter<>(new OAuth1ResponseProperties())
