...
abstract class OAuth2PropertyModel extends PropertyModel implements OAuth2PropertyMatcher {
     ...
     */
    public <T extends OAuth2PropertyModel> T match(GrantType grantType, FlowStep step) {
        validate(grantType, step);
        Map<FlowStep, Constraint> flows = this.mapping.get(grantType);
        if (flows == null || !flows.containsKey(step)) {
            return null;
        }
        return this.duplicate().constraint(flows.get(step));
    }

    @SuppressWarnings("unchecked")
    @Override
    public <T extends OAuth2PropertyModel> T match(GrantType grantType) {
        return this.mapping.containsKey(grantType) ? (T) this : null;
    }
